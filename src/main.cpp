...
private:
    ...
    // ---- Denoiser images ----
    // guide  (rgba32f): xyz = primary normal, w = depth (0 = sky) -- edge-stopping data
    ...
    struct DenoisePC { int32_t sizeX, sizeY, step, finalPass; float exposure; int32_t firstPass; };
    ...
    static LRESULT CALLBACK wndProc(HWND h, UINT msg, WPARAM wp, LPARAM lp) {
        ...
        case WM_KEYDOWN:
            ...
            if (wp == 'D' && self) self->denoiseEnabled = !self->denoiseEnabled;
            return 0;
        case WM_LBUTTONDOWN:
            if (self) {
                self->dragging = true;
    ...
    void createWindow(HINSTANCE hInst) {
        ...
        hwnd = CreateWindowEx(0, wc.lpszClassName,
            "Vulkan Hardware Path Tracing", style,
    ...
    void createScene() {
        ...
        addFloor(20.0f, 0.18f);   // low gloss: keeps the checker contrast under the bright sky
        ...
        // and the albedo must match the NEE light colour in raygen.rgen (Le).
    ...
    void createStorageImage() {
        ...
        // ---- Denoiser images: guide, albedo, ping, pong ----
        auto makeImage = [&](VkFormat fmt, VkImage& img, VkDeviceMemory& mem, VkImageView& view) {
            VkImageCreateInfo ci = aci;                 // same size/usage template
            ci.format = fmt;
            ci.usage  = VK_IMAGE_USAGE_STORAGE_BIT;
            vkCheck(vkCreateImage(dev, &ci, nullptr, &img), "create denoise image");
            VkMemoryRequirements r;
            vkGetImageMemoryRequirements(dev, img, &r);
            VkMemoryAllocateInfo mi{ VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO };
            mi.allocationSize = r.size;
            mi.memoryTypeIndex = findMemoryType(r.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
            vkCheck(vkAllocateMemory(dev, &mi, nullptr, &mem), "alloc denoise image");
            vkBindImageMemory(dev, img, mem, 0);
            VkImageViewCreateInfo vi = avci;
            vi.image = img;
            vi.format = fmt;
            vkCheck(vkCreateImageView(dev, &vi, nullptr, &view), "denoise view");
        };
        makeImage(VK_FORMAT_R32G32B32A32_SFLOAT, guideImage,  guideMem,  guideView);
        makeImage(VK_FORMAT_R8G8B8A8_UNORM,      albedoImage, albedoMem, albedoView);
        makeImage(VK_FORMAT_R32G32B32A32_SFLOAT, pingImage,   pingMem,   pingView);
        makeImage(VK_FORMAT_R32G32B32A32_SFLOAT, pongImage,   pongMem,   pongView);
    ...
    void createDescriptors() {
        ...
        // Sized for the RT set (5 storage images) plus three denoiser sets
        // (5 storage images each, allocated in createDenoiser).
        ...
        writes[8].dstSet = descSet; writes[8].dstBinding = 8; writes[8].descriptorCount = 1;
        writes[8].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE; writes[8].pImageInfo = &albInfo;
    ...
    void createDenoiser() {
        ...
        VkImageView ins [3] = { accumView, pingView, pongView };
        ...
        writes.reserve(15);
        for (int s = 0; s < 3; ++s) {
            VkImageView views[5] = { ins[s], outs[s], guideView, storageView, albedoView };
            for (uint32_t b = 0; b < 5; ++b) {
                infos.push_back({ VK_NULL_HANDLE, views[b], VK_IMAGE_LAYOUT_GENERAL });
    ...
    void drawFrame() {
        ...
        if (denoiseEnabled) {
            ...
            for (VkImage img : { accumImage, guideImage, albedoImage })
            ...
            // -> ping -> (pong + tone-mapped display image).
            ...
            const uint32_t gx = (swapExtent.width  + 7) / 8;
            const uint32_t gy = (swapExtent.height + 7) / 8;

            for (int i = 0; i < 4; ++i) {
                DenoisePC pc{ (int32_t)swapExtent.width, (int32_t)swapExtent.height,
                              steps[i], i == 3 ? 1 : 0, /*exposure*/ 1.0f, i == 0 ? 1 : 0 };
        ...
        // or by the raygen shader directly when the denoiser is toggled off),
        // swapchain image -> TRANSFER_DST.
        imageBarrier(cmd, storageImage,
            VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
            VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT,
            denoiseEnabled ? VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT
                           : VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR,
